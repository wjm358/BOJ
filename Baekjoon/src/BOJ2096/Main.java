...
public class Main {
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 1; i < n; i++) {
			...
			max[i][1] = Math.max(max[i - 1][0] + arr[i][1],
					Math.max(max[i - 1][1] + arr[i][1], max[i - 1][2] + arr[i][1]));
			max[i][2] = Math.max(max[i - 1][1] + arr[i][2], max[i - 1][2] + arr[i][2]);
		}

		for (int i = 1; i < n; i++) {
			min[i][0] = Math.min(min[i - 1][0] + arr[i][0], min[i - 1][1] + arr[i][0]);
			min[i][1] = Math.min(min[i - 1][0] + arr[i][1],
					Math.min(min[i - 1][1] + arr[i][1], min[i - 1][2] + arr[i][1]));
